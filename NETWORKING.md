...
### Host-side DSCP Marking (not yet configured)

For the switch QoS to take full effect, the Mellanox CX4 Lx NICs must mark traffic at the host:

| Traffic type | Required DSCP | Required PCP |
|-------------|--------------|-------------|
| RoCEv2 data | 26 (AF31) | 3 |
| CNP (Congestion Notification Packets) | 48 (CS6) | 6 |

This requires kernel DCB/QoS configuration via `tc` and `dcb` tools (or `mlnx_qos`). Since RDMA workloads run inside Kubernetes pods via the `rdma-roce` NetworkAttachmentDefinition, the marking should be applied to the `bond1.292` interface on each host.

Without this, the switch falls back to treating all RDMA traffic as best-effort (TC1) since DSCP 0 doesn't match any profile. PFC will still be negotiated via LLDP DCBX once the NICs are configured.
